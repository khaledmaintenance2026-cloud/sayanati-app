...
class MaintenanceCompletedScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // الحذف النهائي قرار صريح من الإدارة: مسؤول الصيانة أو المدير فقط، لا
    // الفني — راجع نفس القيد الملزم فعليًا على السيرفر
    // (requireRole('maintenance_manager') في DELETE /api/work-orders/:id).
    // onDelete: null يخفي زر الحذف تمامًا عن بطاقة الفني (راجع MaintenanceReportCard
    // — الزر لا يُرسَم إطلاقًا إن كان onDelete فارغًا) بدل إظهاره ثم رفض
    // السيرفر الطلب برسالة خطأ بعد الضغط عليه.
            ...
            : ListView.separated(
                ...
                itemBuilder: (context, i) {
                  ...
                  return MaintenanceReportCard(
                    report: r,
                    onDelete: canManage ? () => _confirmDelete(context, r.id, '${r.equipment} — ${r.line}') : null,
                  );
                },
              ),
      ),
